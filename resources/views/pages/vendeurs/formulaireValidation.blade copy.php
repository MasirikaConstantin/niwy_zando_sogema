													...
													<tbody>
														...
														@forelse ($vendeur->places as $place)
															...
															<tr>
																...
																<td>
																	{{-- {{$place->pivot->maitre_cube}} --}}

																	<input type="text" name="maitreCube{{$loop->index}}" id="maitreCube{{$loop->index}}" @if( !is_null($place->pivot->maitre_cube_retenu) && $place->pivot->maitre_cube_retenu !== "0" ) value="{{$place->pivot->maitre_cube_retenu}}" @else value="{{$place->pivot->maitre_cube}}" @endif {{ $place->pivot->decision == "valider" ||  $place->pivot->decision == "rejeter" ? 'disabled' : '' }} class="form-control myInputTail" style="max-width: 80px" />
																</td>
																<td>
