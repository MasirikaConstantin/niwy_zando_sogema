        ...
        <div class="col-sm-12 col-md-12 col-lg-10">
            ...
            <div class="card">
                ...
                <button type="button" class="btn btn-primaryM colorText bg_background" style="font-size: 17px;">
                    Total&nbsp;
                    <span class="badge badge-light" style="font-size: 18px;">{{ $emplacements->count() }}</span>
                </button>
                                        ...
                                        <div class="modal-content">
                                            <div class="modal-body">
                                                <div class="form-header">
                                                    <h3 style="text-align: center">Modification emplacement</h3>
                                                </div>
                                                <br />
                                                <div class="modal-btn delete-action">
                                                    <form id="formSendModif{{$item->id}}" action="{{ route('emplacement.update',$item->id) }}" method="POST">
                                                        @csrf
                                                        <div class="row mb-4">
                                                            <div class="col-sm-12">
                                                                ...
                                                                <select name="pavillonId" id="pavillonId" class="form-control" required>
                                                                    ...
                                                                    @forelse ($pavillons as $pv)
                                                                        <option value="{{$pv->id}}" {{$pv->id == $item->pavillon_id ? 'selected' : ''}}>{{$pv->numero}} / Niveau {{$pv->niveau}}</option>
                                                                    @empty                                                                    
                                                                    @endforelse
                                                                </select>
                                                                @error('pavillonId')
                                                            ...
                                                            <div class="col-sm-12">
                                                                ...
                                                                <select name="placeId" id="placeId" class="form-control" required>
                                                                    ...
                                                                    @forelse ($places as $pl)
                                                                        <option value="{{$pl->id}}" {{$pl->id == $item->place_id ? 'selected' : ''}}>{{$pl->nom}} @if(!empty($pl->type_place_id)) {{$pl->typePlace->nom}} {{$pl->typePlace->dimension}} @endif Orientation {{$pl->orientation}} / {{$pl->prix}} $</option>
                                                                    @empty                                                                    
                                                                    @endforelse
                                                                </select>
                                                                @error('placeId')
                                                        ...
                                                        <div class="row mb-4">
                                                            <div class="col-sm-12">
                                                                ...
                                                                <input type="text" name="numero" id="numero" value="{{$item->numero}}" class="form-control" placeholder="Saisir le numéro" required />
                                                                @error('numero')
                                                                    <div class="text-danger">{{ $message }}</div>
                                                                @enderror
                                                            </div>
